...
import { Meal, MealComponent, createOrUpdateMeal, getMealForToday } from "../util/database";
...
function IntakeMeal() {
    ...
    const setPreBolusGiven = (given: boolean) => {
        if (!meal) return;
        setIsLoading(true);
        try {
            createOrUpdateMeal(db, { id: meal?.id, preMealBolusGiven: given });
    ...
    const setAfterBolusGiven = (given: boolean) => {
        if (!meal) return;
        setIsLoading(true);
        try {
            createOrUpdateMeal(db, { id: meal?.id, afterMealBolusGiven: given, mealComponents: meal?.mealComponents });
    ...
    const updateMealComponent = (index: number, c: MealComponent) => {
        if (meal) {
            setMeal({
                ...meal,
                mealComponents: [...meal.mealComponents.slice(0, index), c, ...meal.mealComponents.slice(index + 1)],
            });
        }
    };
    ...
    const afterMealBolus = useMemo(() => {
        if (meal?.mealComponents.every((c) => !!c.eaten)) {
            const totalCarbsEaten = meal.mealComponents.reduce((acc, c) => acc + c.eaten! * c.carbsPerPiece!, 0);
                ...
                <>
                    <section className="mb-24">
                        ...
                        <div className="flex items-center justify-between gap-4">
                            <div>
                                <span style={{ fontWeight: "bold" }}>{meal.preMealBolus}</span> kh über X Symbol
                                eingeben
                            </div>
                            <button onClick={() => setPreBolusGiven(true)}>
                    ...
                    <section className="mb-6">
                        ...
                        {afterMealBolus && (
                            <div className="flex items-center justify-between gap-4">
                                {afterMealBolus > 0 ? (
                                    <div>
                                        <span style={{ fontWeight: "bold" }}>{afterMealBolus}</span> kh über X Symbol
                                        eingeben
                                    </div>
                                ) : (
                                    <div>
                                        ...
                                        Theo muss noch{" "}
                                        <span style={{ fontWeight: "bold" }}>{Math.abs(afterMealBolus)}</span>{" "}
                                        Traubenzucker{" "}
